<template>
  <div class="homeWrapper">
    ...
    <main class="mainContent">
      ...
      <template v-else-if="this.showLiked">
        <Liked :likedCounter="likedCounter" :likedProducts="likedProducts" />
      </template>
    </main>
    <!-- <ResetPassword /> -->
    <Footer />
...
export default {
  ...
  computed: {
    ...
    likedProducts() {
      return this.products.filter((el) => el.liked);
    },
    // searchItems() {
  ...
  methods: {
    ...
    likedToggle(id) {
      this.products.filter((el) => {
        if (el.id === id) {
          el.liked = !el.liked;
          this.likedProductsArray.push(el);
        }
        return true;
      });
    },
    likedProductsCounter() {
      // let result = 0;
      // this.products.forEach((el) => {
      //   if (el.liked) {
      //     result += 1;
...
.mainHeader {
  ...
  display: grid;
  gap: 44px;
  padding: 18px 159px 13.83px 144px;
...
.sortWrapper {
  ...
  display: grid;
  grid-auto-flow: column;
  padding: 12px 224px 12px 11px;
  border-radius: 4px;
  margin-bottom: 15px;
  background: #ffffff;
  box-shadow: 0px 1px 4px 0px #00000037;
  & input {
    width: 143px;
    background: #f9fafb;
    border-radius: 5px;
